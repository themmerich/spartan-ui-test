...
import { BKGResponse } from '@components/form-address/model/bkg-response';

@Injectable({
    providedIn: 'root',
})
export class AdresseSucheService {
    private readonly geoDatenSuggestUrl = SERVER_API_URL + 'api/geodaten/suggest?query=';
    ...
    private convertBKGResponse(response: BKGResponse[]): any {
        ...
        for (const bkg of response) {
            if (bkg.type === 'Haus' || bkg.type === 'Strasse' || bkg.type === 'Ort') {
                ...
                const match = regex.exec(suggestion);
                if (match?.groups) {
                    result.push({
